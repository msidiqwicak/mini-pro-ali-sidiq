...
export const getAvailablePoints = async (userId: string) => {
...
export const calculatePointsRedemption = (
  points: Array<{ id: string; amount: number }>,
  pointsToUse: number
): Array<{ pointId: string; amountUsed: number }> => {
  const plan: Array<{ pointId: string; amountUsed: number }> = [];
