...
export const getRevenueByDay = async (organizerId: string, days = 30) => {
  const from = new Date();
...
export const getRevenueByMonth = async (organizerId: string, year?: number) => {
...
export const getRevenueByYear = async (organizerId: string) => {
  const transactions = await prisma.transaction.findMany({
...
export const getOrganizerStats = async (organizerId: string) => {
  const [totalEvents, totalRevenue, totalAttendees, recentTransactions] =
    await Promise.all([
