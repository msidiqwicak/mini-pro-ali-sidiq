...
import { findTransactionsByUser, findActivePointsByUser, } from "../repositories/transaction.repository.js";
 ...
 */
export const createTransactionService = async (userId, input) => {
    return prisma.$transaction(async (tx) => {
        ...
        if (remaining < input.quantity)
            throw new Error(`Sisa kursi tidak mencukupi. Tersedia: ${remaining} tiket`);
        ...
        if (input.pointsToUse > 0) {
            ...
            const maxUsable = Math.min(input.pointsToUse, totalAvailable, priceAfterDiscount // Can't use more points than remaining price
            );
            if (maxUsable > 0) {
 ...
 */
export const payTransactionService = async (transactionId, userId) => {
    ...
    try {
        ...
        if (!firstTicket) {
            ...
            // Send ticket email
            await sendTicketEmail(transaction.user.email, transaction.user.name, transaction.event.name, firstTicket.qrCode, qrCodeDataUrl, transaction.tickets.length, updatedTransaction.finalAmount);
            // Send payment confirmation
            await sendPaymentConfirmation(transaction.user.email, transaction.user.name, updatedTransaction.finalAmount, transactionId, transaction.event.name);
        ...
        console.warn(`⚠️ Email gagal dikirim untuk transaksi ${transactionId}:`, emailError);
    }
